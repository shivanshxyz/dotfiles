...
// WEAVA code
var messageChannel = document.getElementById("gistnote-pdf-render-msg");
var renderMsgTimeout = null;
var renderedPagesNumber = [];
var cachedHighlightsByPage = {};
var cachedColorCodes = null;
var printingWithHighlights = false;
var loaded = false;

function sendOpenWebsiteMsg(url) {
  var customEvent = document.createEvent("Event");
  customEvent.initEvent("onPDFMessage", true, true);
  customEvent.data = {
    id: "OPEN_WEBSITE",
    url: url,
  };
  messageChannel.dispatchEvent(customEvent);
}

function sendRenderMsg(pageNumber) {
  renderedPagesNumber.push(pageNumber);
  if (renderMsgTimeout) {
    clearTimeout(renderMsgTimeout);
  }
  ...
  renderMsgTimeout = setTimeout(function () {
    _do(renderedPagesNumber);
    renderedPagesNumber = [];
  }, 200);
}

messageChannel.addEventListener("onExtensionMessage", function (event) {
  var data = event.data;
  switch (data.id) {
    case "READY":
      if (!loaded) {
        webViewerLoad(data.isInsideWebApp);
        loaded = true;
      }
      break;

    case "GO_TO_HIGHLIGHT":
      if (PDFViewerApplication.page !== parseInt(data.page)) {
        PDFViewerApplication.page = parseInt(data.page);
      }
      break;
    case "CACHE_HIGHLIGHTS":
      cachedHighlightsByPage = {};
      // group highlights by page
      if (data.highlights) {
        var highlightIds = Object.keys(data.highlights);
        for (var i = 0; i < highlightIds.length; i++) {
          var highlight = data.highlights[highlightIds[i]];
          // pdfPageNumber
          if (!cachedHighlightsByPage[highlight.pdfPageNumber]) {
            cachedHighlightsByPage[highlight.pdfPageNumber] = {};
          }
          cachedHighlightsByPage[highlight.pdfPageNumber][highlightIds[i]] = highlight;
        }
      }
      cachedColorCodes = data.colorCodes;
      // console.log('cached', data.highlights, cachedHighlightsByPage);
      break;
  }
});
...
(function () {
          ...
          function () {
            ...
            if (valueType !== defaultType) {
              if (valueType === "number" && defaultType === "string") {
                ...
                throw new Error(
                  "Preferences_set: '" + value + "' is a \"" + valueType + '", expected "' + defaultType + '".'
                );
              }
            } else {
  ...
  (function (root, factory) {
    ...
    function attachDOMEventsToEventBus(eventBus) {
      ...
      eventBus.on("textlayerrendered", function (e) {
        sendRenderMsg(e.pageNumber);
        console.log("textlayerrendered", e.pageNumber);
        var event = document.createEvent("CustomEvent");
      ...
      eventBus.on("pagechange", function (e) {
        console.log("pagechange", e.pageNumber);
        sendRenderMsg(e.pageNumber);
      ...
      eventBus.on("pagesinit", function (e) {
        setTimeout(function () {
          console.log("pageinit", e.source._currentPageNumber);
          sendRenderMsg(e.source._currentPageNumber);
        }, 1500);
        var event = document.createEvent("CustomEvent");
      ...
      PDFLinkService.prototype = {
        ...
        navigateTo: function PDFLinkService_navigateTo(dest) {
          ...
          var goToDestination = function (destRef) {
            ...
            var pageNumber =
              destRef instanceof Object ? self._pagesRefCache[destRef.num + " " + destRef.gen + " R"] : destRef + 1;
            if (pageNumber) {
  ...
  (function (root, factory) {
    ...
    var TEXT_LAYER_RENDER_DELAY = 1; // ms
      ...
      PDFPageView.prototype = {
        ...
        draw: function PDFPageView_draw() {
          ...
          if (this.textLayerFactory) {
            ...
            // console.log(this.textLayerFactory.createTextLayerBuilder);
              ...
              if (textLayer) {
                self.pdfPage
                  ...
                  .then(function textContentResolved(textContent) {
                    // TODO WEAVA set text layer content
        ...
        // TODO WEAVA handle before print
        beforePrint: function PDFPageView_beforePrint(printContainer) {
          ...
          var canvasWrapper = document.createElement("div");
          canvasWrapper.id = "printPageContainer" + (pdfPage.pageIndex + 1);
          canvasWrapper.appendChild(canvas);
          ...
          if (this.textLayerFactory) {
            ...
            textLayer = this.textLayerFactory.createTextLayerBuilder(textLayerDiv, pdfPage.pageIndex, viewport, false);
          }
          ...
          canvas.mozPrintCallback = function (obj) {
            ...
            pdfPage.render(renderContext).promise.then(
              function () {
                if (printingWithHighlights) {
                  var isBInsideA = function (rectA, rectB) {
                    var horizontal = rectB.x <= rectA.x + rectA.width * 0.7;
                    var vertical = rectB.y <= rectA.y + rectA.height * 0.7;
                    return horizontal && vertical;
                  };

                  // pdf page rendered, get text
                  pdfPage
                    .getTextContent({
                      normalizeWhitespace: true,
                    })
                    .then(function textContentResolved(textContent) {
                      ...
                      textLayer.render(TEXT_LAYER_RENDER_DELAY);
                      setTimeout(function () {
                        // Text layer rendered, add annotation

                        // get the highlights of this page
                        var highlightsOnThisPage = cachedHighlightsByPage[pdfPage.pageIndex + 1];
                        if (highlightsOnThisPage) {
                          var highlightIdsOnThisPage = Object.keys(highlightsOnThisPage);
                          for (var j = 0; j < highlightIdsOnThisPage.length; j++) {
                            var highlight = highlightsOnThisPage[highlightIdsOnThisPage[j]];
                            // get the range
                            var searchEl = document.getElementById("printPageContainer" + (pdfPage.pageIndex + 1));
                            var range = GistNoteDomSearch.searchRange(
                              highlight.text,
                              highlight.startStr,
                              highlight.endStr,
                              searchEl
                            ...
                            if (!range) {
                              ...
                              var colorCode = cachedColorCodes[highlight.colorNum - 1];
                              ctx.fillStyle = colorCode;
                              for (var k = 0; k < rects.length; k++) {
                                var rect = rects[k];
                                var inside = false;
                                for (var l = 0; l < k; l++) {
                                  if (isBInsideA(rects[l], rect)) {
                                    inside = true;
                                    break;
                                  }
                                }
  ...
  (function (root, factory) {
    ...
    function configure(PDFJS) {
      ...
      PDFJS.workerSrc = "../build/pdf.worker.build.js";
    ...
    var PDFViewerApplication = {
      ...
      zoomFull: function pdfViewZoomFull() {
        var fullZoomCheckbox = document.getElementById("fullscreen-toggle-checkbox");
        var isChecked = fullZoomCheckbox.checked;
        if (isChecked) {
        ...
        return loadingTask.promise.then(
          ...
          function getDocumentError(exception) {
            ...
            var loadingErrorMessage = mozL10n.get(
              "loading_error",
              null,
              "An error occurred while loading the PDF, the PDF URL or content may have been changed."
            );
            ...
            if (exception instanceof pdfjsLib.InvalidPDFException) {
              ...
              loadingErrorMessage = mozL10n.get(
                "invalid_file_error",
                null,
                "Invalid or corrupted PDF file, the PDF URL or content may have been changed."
              );
              ...
              loadingErrorMessage = mozL10n.get(
                "missing_file_error",
                null,
                "Missing PDF file, the PDF URL or content may have been changed."
              );
            } else if (exception instanceof pdfjsLib.UnexpectedResponseException) {
              loadingErrorMessage = mozL10n.get(
                "unexpected_response_error",
                null,
                "Unexpected server response, the PDF URL or content may have been changed."
              );
            }
      ...
      download: function pdfViewDownload() {
        // MessageSender.sendToWebAppAction("download");
      ...
      error: function pdfViewError(message, moreInfo) {
        ...
        if (/file:[\/]+/i.test(urlLocation)) {
          fileUrl = urlLocation.split("?file=")[1].replace("&webapp=true", "");
          if (file.length < fileUrl.length) {
            file = fileUrl;
          }
        }

        console.error(message, file);
        MessageSender.sendLogEventMessage("PDF_ERROR", {
          file: file,
          message: message,
          moreInfoMessage: moreInfo.message,
          errorStatus: moreInfo.exception ? moreInfo.exception.status : "",
        });

        var moreInfoText = "";
        if (file.indexOf("file://") == 0) {
          moreInfoText = "The PDF link may have been changed, please double check:" + file;
        } else {
          if (moreInfo.exception.status == "404") {
            moreInfoText =
              "(404) The PDF link may have been changed, please click the button above for details from the host.";
          } else if (moreInfo.exception.status == "403") {
            moreInfoText =
              "(403) Login is required for viewing the PDF, please click the button above for details  from the host.";
          } else if (moreInfo.exception.status == "500") {
            moreInfoText = "(500) Please click the button above for details from the host.";
          } else {
            moreInfoText =
              mozL10n.get(
                "error_version_info",
      ...
      load: function pdfViewLoad(pdfDocument, scale) {
        ...
        Promise.all([onePageRendered, this.animationStartedPromise]).then(function () {
          pdfDocument.getOutline().then(function (outline) {
            if (outline) {
              document.getElementById("outline-button-wrapper").className = "";
            }
    ...
    function webViewerInitialized() {
      ...
      appConfig.toolbar.zoomFull.addEventListener("click", function () {
        PDFViewerApplication.zoomFull();
      });
      ...
      appConfig.toolbar.print.addEventListener("click", function (e) {
        printingWithHighlights = false;
        // PDFViewerApplication.eventBus.dispatch('print');
      });
      // TODO WEAVA
      appConfig.toolbar.downloadWithHighlights.addEventListener("click", function (e) {
        printingWithHighlights = true;
        PDFViewerApplication.eventBus.dispatch("print");
      });
      appConfig.toolbar.printWithHighlights.addEventListener("click", function (e) {
        printingWithHighlights = true;
        PDFViewerApplication.eventBus.dispatch("print");
      });

      appConfig.toolbar.download.addEventListener("click", function (e) {
        if (!appConfig.isInsideWebApp) {
    ...
    function webViewerOpenFileViaURL(file) {
      //TODO WEAVA
      if (!file) {
        // set the open file  modal visible
      }

      file = decodeURIComponent(file);
      if (file.indexOf("pdfstorage") !== -1) {
        // handle weavastorage reload
        if (file.indexOf("token") == -1) {
          sendOpenWebsiteMsg(file);
          return;
        }

        // history.replaceState({}, '', location.href.split('?token')[0]);
        history.replaceState({}, "", location.href.split(encodeURIComponent("?token"))[0]);
        ...
        //     PDFViewerApplication.open(xhr.responseURL);
        //   },
        //   error: function (request, textStatus, errorThrown) {
        //     console.log('error', 'cannot get signedurl');
        //   }
        // });
        return;
      }
      ...
      if (file && file.lastIndexOf("file:", 0) === 0) {
        chrome.extension.isAllowedFileSchemeAccess(function (allow) {
          console.log(allow);
          if (!allow) {
            setTimeout(function () {
              SettingStoreActions.showLocalAccessModal();
            }, 1000);
          } else {
...
function getViewerConfiguration() {
  return {
    ...
    toolbar: {
      ...
      zoomFull: document.getElementById("fullscreen-toggle"),
      viewFind: document.getElementById("viewFind"),
      openFile: document.getElementById("openFile"),
      // TODO WEAVA
      print: document.getElementById("print-origin-button"),
      printWithHighlights: document.getElementById("print-with-highlights-button"),
      presentationModeButton: document.getElementById("presentationMode"),
      download: document.getElementById("download-origin-button"),
      downloadWithHighlights: document.getElementById("download-with-highlights-button"),
...
function webViewerLoad(isInsideWebApp) {
  var config = getViewerConfiguration();
  config.isInsideWebApp = isInsideWebApp;
